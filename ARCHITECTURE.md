...
### 3d. Action-Conditioned Pointer Execution (Autoregressive Actor)
The actor policy utilizes a sequence-to-sequence decoder to pick from the dynamic action masks generated headlessly in Java. Because an action in MegaMek (like a `MovePath`) is heavily contextual—a single destination Hex may have dozens of pathing permutations involving differing MP costs, movement modes, and final facings—the network cannot simply point to raw spatial Hex nodes. Instead, it must point to **Action-Conditioned Embeddings**.

* **Initialization:** The decoder's internal hidden state $s_0$ is initialized using the global environment representation $z$.
* **Action Embedding Formulations ($e_a$):** For each valid sub-action combination presented in the engine's mask, a dynamic representation token is evaluated by fusing the context of the related HAN nodes and the discrete path/route metadata vectors:
$$e_{action\_j} = \text{ActionMLP}( H_{unit} \oplus H_{target} \oplus \text{action\_metadata\_vector} )$$
*(Where `metadata` encodes variables like `is_jump`, `mp_used`, `final_facing`, or for attack masks, `to_hit_modifier` and `weapon_type`.)*
* **Scoring/Attention:** To choose an action token during autoregressive steps, the decoder computes dot-product attention scores against these instantiated action-embedding matrices rather than the raw spatial graph:
$$\text{Logits}_j = s_k^T \cdot e_{action\_j}$$
* **Action Masking:** Unprovided actions unapproved by the deterministic MegaMek rule engine are natively absent from the calculation blocks, removing the risk of the network sampling out-of-bounds inputs.
