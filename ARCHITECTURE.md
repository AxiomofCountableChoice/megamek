...
1. Edges for showing two hexes are adjacent, and which edge the connect via ($i = 0, \cdots 5$)
...
Defining the sets
$$ \mathcal{V} = \mathcal{V}_U \cup \mathcal{V}_W \cup \mathcal{V}_H, \quad \mathcal{E} = \bigcup_{e \in \mathcal{T}} \mathcal{E}_{e}, \quad \mathcal{T} = \{\text{hexAdj}, \text{occupies}, \text{equips}, \cdots \} $$
...
To guarantee a competitive Nash Equilibrium and prevent cooperative local minima (e.g., a ``Truce'' where agents refuse to act to avoid penalties), the environment strictly enforces a Zero-Sum constraint at every time-step: $\mathcal{R}^{(1)}_t = -\mathcal{R}^{(2)}_t$. Further-more, while the ideal reward (to match the true game reward and Nash Equilibrium) is just the win-loss state, given how sparse the rewards are, we introduce auxiliary dense rewards.
...
Let $\mathbb{I}$ denote an indicator function. Tertiary penalties shape behaviour strictly around rule-set de-buffs or unforced Piloting Skill Rolls (PSRs). Any penalty incurred by Player 1 is explicitly awarded to Player 2, preserving the zero-sum mirror and natively rewarding the offensive use of heat-inducing weapons (e.g., Flamers) - for the $i$-th unit for player $\eta \in \{1, 2\}$ on the $t$-th round:
$$ \text{TP}^{(\eta)}_{i, t} = \epsilon_{mov}\mathbb{I}_{\{Heat \ge 5\}} + \epsilon_{acc}\mathbb{I}_{\{Heat \ge 8\}} + \epsilon_{shut}\mathbb{I}_{\{Heat \ge 14\}} + \epsilon_{ammo}\mathbb{I}_{\{Heat \ge 19 \land Ammo\}} + \epsilon_{psr}\mathbb{I}_{\{UnforcedPSR\}} $$
...
For discrete node types $\tau(n)$ and edge types $\phi(\varepsilon)$ select specific learned weight matrices to dynamically route messages. For a directed edge $\varepsilon_{s,\iota}$ from source $n_s$ to target $n_{\iota}$:
...
&= \text{GlobalAttentionPooling}\left(\{h_{n_i}\}_{n_i \in \mathcal{V}_{b_t}}\right)\\
...
$$ e_{a_k} = \text{ActionMLP}( h_{\text{active\_unit}} \oplus h_{\text{target\_node}} \oplus x_{a_k} ) \quad \forall a_k \in \mathcal{V}_{\mathcal{A}_g^{\eta}} $$
where $x_{a_k}$ represents features that we compute for the given sub-action $a_k$, and $\mathcal{V}_{\mathcal{A}_g^{\eta}}$ is the set of nodes in the tree defined by $\mathcal{A}_{g_t}^{\eta}$. The causal transformer processes the latent state $z_t$ and prefix embeddings to generate a query vector $s_k$. Policy logits are evaluated via a pointer-network dot product:
$$ s_k = \text{TransformerDecoder}\left(z_t, [e_{a_0}, \dots, e_{a_{k-1}}] \right), \quad \text{Logits}(a_k) = s_k^T \cdot e_{a_k}. $$
Letting $\varphi(a)$ be the index of the terminal node for action $a$, and $\mathcal{C}(a)$ be the children of node $a$, this then defines the actor policy as:
$$ \pi_{\theta}(a | g_t) = \prod_{k=1}^{\varphi(a)} \frac{\exp\left(s_{k}^T \cdot e_{a_k}\right)}{\sum_{a_j \in \mathcal{C}(a_{k-1})} \exp\left(s_{k}^T \cdot e_{a_j}\right)} $$
...
$$ \{g_{t,i}, a_{t,i}, r_{t,i}\}_{t = 1}^{T_i} \sim \mu_i $$
generated under some behaviour policy $\mu_i$, for $i \in 1, \cdots, N$, and $\hat{\mathbb{E}}$ is the empirical expectation over samples across these trajectories from some replay-buffer capturing them.

We note that the $\mu_i$ in our case will be the actor network defined above, but instantiated with an older iteration of the model parameters, call them $\tilde{\theta}$, that is $\mu_i = \pi_{\tilde{\theta}}$, similarly the value functions used in the V-trace, they utilise prior iteration parameters $\tilde{\omega}$. For simplicity in the following, we drop the explicit references to each trajectory sample $i$.
...
where $\delta_k V = \bar{\rho}_k (r_k + \gamma \bar{V}_\omega(g_{k+1}) - \bar{V}_\omega(g_k))$, $\bar{\rho}_k = \min\left(\bar{\rho}, \frac{\pi_{\theta}(a_k|g_k)}{\mu(a_k|g_k)}\right)$, and $c_j = \min\left(\bar{c}, \frac{\pi_{\theta}(a_j|g_j)}{\mu(a_j|g_j)}\right)$, and $\bar{\rho}, \bar{c} \in (0,1)$ and $\bar{c} \leq \bar{\rho}$. In particular we see the following recursive representation
...
$$ \nabla_{\theta} \hat{L}_{actor}(\theta) = -\hat{\mathbb{E}} \left[ \rho_t \nabla_{\theta} \left[ \log \pi_\theta(a_t|g_t) \right] \left( r_t + \gamma \left\{v_{t+1}(\omega) + \lambda \sqrt{\sigma_{\omega}^2(g_{t+1})} \right\} - \bar{V}_\omega(g_t) \right) \right]. $$
...
Prior to RL training, the Actor network undergoes Behavioural Cloning via Teacher Forcing. The objective is the Segmented Cross-Entropy Loss applied independently at each sequence step and summed over the hierarchy length $K_t$:
$$ \mathcal{L}_{BC} = \hat{\mathbb{E}} \left[ -\sum_{k=1}^{K_t-1} \log \left( \frac{\exp((s_{t,k})^T \cdot e_{a_{t,k}^{\ast}})}{\sum_{a_j \in \mathcal{C}(a_{t,k-1}^{\ast})} \exp((s_{t,k})^T \cdot e_{a_j})} \right) \right] $$
where $\left\{g_t, a_t = \left\{a_{t,k}^{\ast} \right\}_{k=0}^{K_t-1}\right\}_{t=1}^{T_i}$ are the observations used to compute the loss estimate. This establishes baseline competency in movement, line-of-sight pathing, and weapon bracketing before the IMPALA gradients are engaged.
