...
To solve this, actions are modeled as **Action Nodes ($V_A$)** within the heterogeneous graph, acting as interim roots in a hierarchical decision tree.

#### Definition of Action Nodes ($V_A$)
Action nodes are injected dynamically per-activation and represent the high-level semantic branches of valid choices:
1. **Movement Root Nodes:** Correspond to distinct combinations of `(Target Hex, Movement Mode)`. 
   * *Features:* `[MP_Cost, Expected_TMM, Is_Jump]`. 
   * *Edges:* Directed to the destination `Hex` node.
   * *Autoregressive Children:* Subsequent choices for `Final Facing` and `Specific Path Taken`.
2. **Combat Root Nodes:** Correspond to `(Target Enemy Mech)`.
   * *Features:* `[Range, Expected_To_Hit, Target_Movement_Modifier]`.
   * *Edges:* Directed to the target `Mech` node.
   * *Autoregressive Children:* Subsequent atomic actions for `Weapon 1 assignment`, `Weapon 2 assignment`, etc.
3. **Terminal Nodes ($V_{Terminal}$):** Explicit `<END_PHASE>` or `<DONE>` tokens.
   * *Purpose:* To allow early exit from an autoregressive sequence (e.g., choosing to fire only 2 of 10 weapons and then terminating the combat target assignment, rather than individually looping through the remaining 8 to select `[None]`).

#### The Autoregressive Execution Loop
* **Initialization:** The decoder's internal hidden state $s_0$ is initialized using the global environment representation $z$.
* **Action Embedding Formulations ($e_a$):** For the current tier of the tree (e.g., Movement Roots), a dynamic representation token is evaluated by fusing the Action Node covariates with its designated graph target:
$$e_{action\_j} = \text{ActionMLP}( H_{unit} \oplus H_{target\_hex} \oplus X_{action\_node} )$$
* **Scoring/Attention:** The decoder computes dot-product attention scores against these specific interim root embeddings:
$$\text{Logits}_j = s_k^T \cdot e_{action\_j}$$
* **Descent:** Once an interim root (e.g., a Target Hex) is selected, the decoder steps to $s_{k+1}$ and evaluates the valid atomic children (e.g., the 6 possible facings) using the selected parent embedding as context.

#### Supervised Cross-Entropy Filtering (MCTS Limit)
During Behavioral Cloning, forcing the Cross-Entropy Loss to compute across every possible sub-action branch is computationally restrictive. Instead, the loss evaluates strictly across the sampled "mini MCTS" branches generated by the Offline Engine (i.e. we only instantiate Action Nodes for the subset of highly plausible maneuvers generated by Princess's pathfinder logic, effectively limiting the scope to tactical relevance).
