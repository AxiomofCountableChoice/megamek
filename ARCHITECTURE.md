...
1. **Entity Nodes ($\mathscr{V}_U$):** are entity nodes, which encode information about units partaking in the game. A set of node covariates are of the form:
...
2. **Hex Nodes ($\mathscr{V}_H$):** are hex nodes, which encode information about terrain and battlefield geometry. A set of node covariates are of the form:
   `elevationLevel`, `terrainType_OneHot`, `lightForestFlag`, `heavyForestFlag`, `fire/smokeFlag`, `objectiveVPValue`, `isExtractionZone`, $\dots$
3. **Weapon Nodes ($\mathscr{V}_W$):** are weapon nodes, which encode information about weapons mounted to mechs, vehicles or wielded by infantry. A set of node covariates are of the form:
...
$$ \mathscr{E}_{\text{hexAdj}_i}: \mathscr{V}_H \to \mathscr{V}_H $$
2. Edges for showing what hex a unit occupies.
$$ \mathscr{E}_{\text{occupies}}: \mathscr{V}_U \to \mathscr{V}_H $$
3. Edges for showing what unit a weapon is equipped to.
$$ \mathscr{E}_{\text{equips}}: \mathscr{V}_W \to \mathscr{V}_U $$
4. Edges for showing what hexes a unit can move to.
$$ \mathscr{E}_{\text{moveTypeTMM}_{\text{bracket}_i}}: \mathscr{V}_U \to \mathscr{V}_H $$
5. Edges for showing which hexes an inactivated enemy can move to, which are in movement range of a unit to be activated.
$$ \mathscr{E}_{\text{movementThreat}}: \mathscr{V}_U \to \mathscr{V}_H $$
...
$$ \mathscr{E}_{\text{LOSThreat}}: \mathscr{V}_U \to \mathscr{V}_H $$
7. Edges for showing which enemies are in LOS of a unit to be activated.
$$ \mathscr{E}_{\text{LOSTarget}}: \mathscr{V}_U \to \mathscr{V}_U $$
8. Edges for showing which hexes a unit to be activated have partial cover from already activated enemy units.
$$ \mathscr{E}_{\text{partialCover}}: \mathscr{V}_U \to \mathscr{V}_H $$

Defining the sets:
$$ \mathscr{V} = \mathscr{V}_U \cup \mathscr{V}_W \cup \mathscr{V}_H, \quad \mathscr{E} = \bigcup_{e \in \mathscr{T}} \mathscr{E}_{e}, \quad \mathscr{T} = \lbrace \text{hexAdj}_i, \text{occupies}, \text{equips}, \dots \rbrace $$
we have that the graph $(\mathscr{V}, \mathscr{E})$ represents the game board-state at any given time. However, to capture the meta information, we need as well to have information regarding the current phase, information regarding who had initiative during the turn, etc.

We let the space phase meta-data be $\mathscr{P}$, we note that each element $p \in \mathscr{P}$ is a vector which encodes:
* The turn number
...
Let the space of all valid board-states be $\mathscr{B}$, and let $\mathscr{G} \subset \mathscr{P} \times \mathscr{B}$ be the set of all legal game-states. Then we have that the state of a game can be represented as an element:

$$ (p, B) \in \mathscr{G}, \exists (\mathscr{V}, \mathscr{E}) \in \mathscr{B}: B = (\mathscr{V}, \mathscr{E}). $$
...
We define $\mathscr{A}$ as the space of all actions, and $\mathscr{A}_g$ as the set of legal actions for the given game-state $g \in \mathscr{G}$. This legal action set $\mathscr{A}_g$ has a structure that is a union of disjoint trees, with the root nodes being the unit to activate in the phase, and the nodes being the 'atomic sub-actions', with edges showing the steps between them, and leaf nodes being the `declarationComplete` terminal state. By introducing an arbitrary `declarationState` node, we can create a single tree, with each of the child nodes being a yet-to-be-activated unit. We seek to enumerate the 'sub-actions' per phase now:
...
As a final point, we recognise that the action space is actually defined as the Cartesian product of the two players action spaces, specifically we define $\mathscr{A} = \mathscr{A}^1 \times \mathscr{A}^2$, where the non-active players valid action space is simply $\emptyset$.

Letting the active player be $\eta \in \lbrace 0, 1 \rbrace$, then we have that we represent $\mathscr{A}_g^{\eta}$ as the tree-structure described above, having nodes $ \mathscr{V}_{\mathscr{A}_g^{\eta}}$ representing the set of sub-actions, with edges between consecutive sub-actions being given by the valid action-mask that Megamek generates. We associated $\mathscr{A}_g^{\eta}$ with the valid action mask structure.
...
$$ \mathbb{P}:  \mathscr{G} \times \mathscr{A}^1 \times \mathscr{A}^2 \mapsto \mathscr{M}_{\mathscr{G}} $$

where $\mathscr{M}_{\mathscr{G}}$ is the space of probability measures over $\mathscr{G}$. We provide the following informal justification that this is a valid transition kernel. Conditional on the game state $g \in \mathscr{G}$ and valid action space $\mathscr{A}_g^1 \times \mathscr{A}_g^2$, any declared action $a^1 \times a^2 \in \mathscr{A}_g^1 \times \mathscr{A}_g^2$, will generate either a series of dice-rolls against target numbers that either succeed or fail, or an action completes successfully. This generates a new game-state $g^{\prime}$ according to the successful / failed rolls, leading to the next action to be sampled. This next-state distribution is fully determined by the current game-state $g$ and declared action $a^1 \times a^2$, leading to the Markovian property claimed.

### 1.4 Reward Structure ($R$)
To guarantee a competitive Nash Equilibrium and prevent cooperative local minima (e.g., a "Truce" where agents refuse to act to avoid penalties), the environment strictly enforces a Zero-Sum constraint at every time-step: $\mathscr{R}^{(1)}_t = -\mathscr{R}^{(2)}_t$. Furthermore, while the ideal reward (to match the true game reward and Nash Equilibrium) is just the win-loss state, given how sparse the rewards are, we introduce auxiliary dense rewards.
...
$$ \mathscr{R}^{(1)}_t = \beta_{BV} \left( \frac{\Delta \text{BV}^{(2)}_t - \Delta \text{BV}^{(1)}_t}{\text{TotalMatchBV}} \right) + \beta_{obj} \left( \frac{\Delta \text{VP}_t^{(1)} - \Delta \text{VP}_t^{(2)}}{\text{MaxVP}} \right) + \beta_{TP} \left( \sum_{i \in \texttt{units}_2}\text{TP}^{(2)}_{i, t} - \sum_{j \in \texttt{units}_1} \text{TP}^{(1)}_{j, t} \right) $$
...
$$ \mathscr{M} = \langle \mathscr{G}, \mathscr{A}^1, \mathscr{A}^2, \mathbb{P}, \mathscr{R}, \gamma \rangle $$
...
The state graph $g_t = (b_t, p_t)$, of board state $b_t$ and phase meta-data $p_t$ is processed via an HGT layer. We introduce the nodes $n \in \mathscr{V}_{b_t}$ and edges $\varepsilon \in \mathscr{E}_{b_t}$ as the edges and nodes of the current state graph $b_t$. Let $\tau$ and $\phi$ be the node type and edge type identifier functions and $\mathscr{N}_{b_t}(n)$ represent the 1-hop neighbourhood of node $n$ across all edge types.
...
$$ \text{Attention}(n_s, \varepsilon_{s,\iota}, n_{\iota}) = \underset{\forall n_s \in \mathscr{N}_{b_t}(n_{\iota})}{\text{Softmax}} \left( \frac{K(n_s) W^{ATT}_{\phi(\varepsilon_{s,\iota})} Q(n_{\iota})^T}{\sqrt{d}} \cdot \mu_{\langle \tau(n_s), \phi(\varepsilon_{s,\iota}), \tau(n_{\iota}) \rangle} \right) $$

$$ h_{n_{\iota}}^{(l)} = \text{GELU} \left( \sum_{n_s \in \mathscr{N}_{b_t}(n_{\iota})} \text{Attention}(n_s, \varepsilon_{s,\iota}, n_{\iota}) \cdot \left(V(n_s) W^{MSG}_{\phi(\varepsilon_{s,\iota})}\right) W_{A\text{-}\tau(n_{\iota})} \right) + h_{n_{\iota}}^{(l-1)} $$
...
&= \text{GlobalAttentionPooling}\left(\lbrace h_{n_i}\rbrace_{n_i \in \mathscr{V}_{b_t}}\right)\\
&= \sum_{n_i \in \mathscr{V}_{b_t}} \text{softmax}(W_{gate} h_{n_i}) \odot (W_{feat} h_{n_i})\\
\implies z_t &= z_{graph} \oplus \text{MLP}(p_t).
\end{aligned}
$$

### 2.2 Action-Conditioned Pointer Execution (Actor)
The Actor policy $\pi_{\theta}$ navigates the valid action space $\mathscr{A}_{g_t}^{\eta}$ using an autoregressive causal transformer. Candidates sub-actions are mapped to continuous embeddings via an `ActionMLP`:

$$ e_{a_k} = \text{ActionMLP}( h_{\text{activeUnit}} \oplus h_{\text{targetNode}} \oplus x_{a_k} ) \quad \forall a_k \in V_{\mathscr{A}_g^{\eta}} $$

where $x_{a_k}$ represents features that we compute for the given sub-action $a_k$, and $V_{\mathscr{A}_g^{\eta}}$ is the set of nodes in the tree defined by $\mathscr{A}_{g_t}^{\eta}$. The causal transformer processes the latent state $z_t$ and prefix embeddings to generate a query vector $s_k$. Policy logits are evaluated via a pointer-network dot product:

$$ s_k = \text{TransformerDecoder}\left(z_t, [e_{a_0}, \dots, e_{a_{k-1}}] \right), \quad \text{Logits}(a_k) = s_k^T \cdot e_{a_k}. $$

Letting $\varphi(a)$ be the index for the terminal node of action $a$, and $\mathscr{C}(a)$ be the children of node $a$, this then defines the actor policy as:

$$ \pi_{\theta}(a \mid g_t) = \prod_{k=1}^{\varphi(a)} \frac{\exp\left(s_{k}^T \cdot e_{a_k}\right)}{\sum_{\zeta \in \mathscr{C}(a_{k-1})} \exp\left(s_{k}^T \cdot e_{\zeta}\right)} $$
...
$$ \nabla_{\theta} \hat{L}_{entropy}(\theta) = \hat{\mathbb{E}} \left[ \sum_{a \in \mathscr{A}_{valid}} \nabla_{\theta} \left[ \pi_\theta(a \mid g_t) \log \pi_\theta(a \mid g_t) \right] \right]. $$
...
$$ \mathscr{L}_{BC} = \hat{\mathbb{E}} \left[ -\sum_{k=0}^{K_t-1} \log \left( \frac{\exp((s_{t,k})^T \cdot e_{a_{t,k}^{\ast}})}{\sum_{a_j \in \mathscr{C}(a_{k-1})} \exp((s_{t,k})^T \cdot e_{a_j})} \right) \right] $$
...
* **_Initialization:_** Java sends the static topology ($\mathscr{V}_H$, $\mathscr{E}_{adj}$) once per match. The Python Actor caches this in RAM.
* **_Step Payload (Deltas):_** At each step, Java transmits only the dynamic state ($\mathscr{V}_U$ covariates, $\mathscr{E}_{occ}$, $\mathscr{E}_{LoS}$) and the current hierarchical Action Mask Tree.
