...
### 3d. The Pointer Network (Autoregressive Actor)
The actor policy utilizes a sequence-to-sequence decoder to output the hierarchical action arrays defined in Section 1c.
* **Initialization:** The decoder's hidden state $s_0$ is initialized using the global vector $z$.
* **Spatial Targeting via HAN:** When the sequence requires selecting a Unit or Hex on the board, the decoder computes a dot-product attention score directly against the HAN's output embeddings $H_{updated}$. 
$$\text{Logits} = s_k^T \cdot H_{valid\_nodes}$$
* **Non-Spatial Targets:** For discrete sub-actions, the decoder attends over a small matrix of weapon embeddings generated by a shared Action MLP: $e_{w} = \text{ActionMLP}(x_w)$.
* **Action Masking:** The logits are multiplied by the hierarchical action mask received from the Java engine to strictly zero out invalid branches before the softmax is applied.
