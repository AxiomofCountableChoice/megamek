...
$$
\mathcal{R}^{(1)}_t = \beta_{BV} \left( \frac{\Delta \text{BV}^{(2)}_t - \Delta \text{BV}^{(1)}_t}{\text{TotalMatchBV}} \right) 
+ \beta_{obj} \left( \frac{\Delta \text{VP}_t^{(1)} - \Delta \text{VP}_t^{(2)}}{\text{MaxVP}} \right) 
+ \beta_{TP} \left( \sum_{i \in \texttt{units}_2}\text{TP}^{(2)}_{i, t} - \sum_{j \in \texttt{units}_1} \text{TP}^{(1)}_{j, t} \right) 
$$
...
Letting $\varphi(a)$ be the index of the terminal node for action sequence $a$, and $\mathcal{C}(a_j)$ be the valid children of node $a_j$, this then defines the actor policy as:

$$
\pi_{\theta}(a \mid g_t) = \prod_{k=1}^{\varphi(a)} \frac{\exp\left(s_{k}^T \cdot e_{a_k}\right)}{\sum_{a_j \in \mathcal{C}(a_{k-1})} \exp\left(s_{k}^T \cdot e_{a_j}\right)}
$$
...
$$
\delta_k V = \bar{\rho}_k \left(r_k + \gamma \bar{V}_\omega(g_{k+1}) - \bar{V}_\omega(g_k)\right)
$$

$$
\bar{\rho}_k = \min\left(\bar{\rho}, \frac{\pi_{\theta}(a_k \mid g_k)}{\mu(a_k \mid g_k)}\right), \quad c_j = \min\left(\bar{c}, \frac{\pi_{\theta}(a_j \mid g_j)}{\mu(a_j \mid g_j)}\right)
$$

with $\bar{\rho}, \bar{c} \in (0,1)$ and $\bar{c} \le \bar{\rho}$. In particular we see the following recursive representation:
...
$$
\nabla_{\theta} \hat{L}_{actor}(\theta) = -\hat{\mathbb{E}} \left[ \rho_t \nabla_{\theta} \left[ \log \pi_\theta(a_t \mid g_t) \right] \left( r_t + \gamma \left[ v_{t+1}(\omega) + \lambda \sqrt{\sigma_{\omega}^2(g_{t+1})} \right] - \bar{V}_\omega(g_t) \right) \right]
$$
...
$$
\mathcal{L}_{BC} = \hat{\mathbb{E}} \left[ -\sum_{k=1}^{K_t-1} \log \left( \frac{\exp \left( s_{t,k}^T \cdot e_{a_{t,k}^{\ast}} \right)}{\sum_{a_j \in \mathcal{C}(a_{t,k-1}^{\ast})} \exp \left( s_{t,k}^T \cdot e_{a_j} \right)} \right) \right]
$$

where $\lbrace g_t, a_t = \lbrace a_{t,k}^{\ast} \rbrace_{k=0}^{K_t-1} \rbrace_{t=1}^{T_i}$ are the observations used to compute the loss estimate. This establishes baseline competency in movement, line-of-sight pathing, and weapon bracketing before the IMPALA gradients are engaged.
